...
const CustomSelect = ({ onChange }: any) => {
   ...
   const options = danceStyles.map((cat) => ({
      value: cat._id,
      label: t(`common.category_list.${cat.name.toLowerCase().replace(/ & /g, '_').replace(/ /g, '_')}`, cat.name),
   }))

   const [selectedDanceStyle, setSelectedDanceStyle] = useState<any>(null)

   const handleDanceStyleChange = (selectedOption: any) => {
      setSelectedDanceStyle(selectedOption)
      if (onChange) {
   ...
   const handleSearchSubmit = (e: React.FormEvent) => {
      ...
      if (selectedDanceStyle) {
         const danceStyle = danceStyles.find(cat => cat._id === selectedDanceStyle.value)
         params.set('dance-style', danceStyle?.slug || selectedDanceStyle.value)
      }
   ...
   return (
      <form onSubmit={handleSearchSubmit} className="tgmenu__search-form">
         ...
         <div className="input-grp">
            <input
               type="text"
               placeholder={t('common.search_courses_placeholder')}
               value={searchText}
