...
const listStudentProfiles = async (req, res, next) => {
  try {
    ...
    if (req.user.role === "teacher") {
      const bookings = await Booking.find({ teacherId: req.user.id }).distinct(
        "studentId",
      );
...
const updateStudentProfile = async (req, res, next) => {
  try {
    ...
    const { photo, phone, timezone, country, state, city, socialLinks } =
      req.body;
    ...
    if (socialLinks) {
      ...
      if (socialLinks.website !== undefined)
        profile.socialLinks.website = socialLinks.website;
    ...
    await profile.save();
    await profile.populate("userId", "name email status");
    res.json({ profile, photo: profile.photo });
  } catch (err) {
    next(err);
  }
};
...
const calculateStudentProgress = async (userId) => {
  try {
    ...
    const totalLessonsBooked = bookings.filter(
    ...
    bookings.forEach((booking) => {
      if (booking.courseId && booking.status !== "cancelled") {
        ...
        progress.progressPercentage = Math.round(
          (progress.completed / progress.totalBooked) * 100,
        );
...
const getWishlist = async (req, res, next) => {
  try {
    ...
    let wishlistCourses = await Course.find({
    ...
    res.json({ wishlist: wishlistCourses });
  } catch (err) {
    next(err);
  }
};
